...
- [x] Kubernetes cluster capable of handling Spark jobs (batch and streaming)  
- [x] Dockerized PySpark streaming application  
- [x] Kafka integration with input/output topics  
- [x] Data transformation (multiply `data_array` by 2)  
- [x] Timestamp filtering (events from last hour)  
- [x] JSON message processing  
- [x] Real-time streaming with micro-batch processing
...
- **KRaft mode:** No ZooKeeper (Strimzi 0.47+)  
- **Replicas:** 2 broker replicas  
- **Replication factor:** 2  
- **Storage:** Ephemeral (dev/test)

### Spark Application

- **Spark:** 3.5.0  
- **Resources:** 2Gi driver, 1Gi executor  
- **Trigger:** 10-second micro-batches  
- **Checkpoint:** `/tmp/checkpoint`  
- **Kafka:** Native 0.10+ connector
...
- Filters events with `timestamp >= now() - 1 hour`  
- Writes transformed JSON to `output-topic`
...
echo "{\"Restaurantid\": 234, \"Event\": \"map_click\", \"Properties\": {\"timestamp\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"is_relevant\": true, \"data_array\": [1.0, 2.3, 2.4, 12.0]}}" | /opt/kafka/bin/kafka-console-producer.sh --topic input-topic --bootstrap-server localhost:9092
...
```bash
kubectl describe kafkatopic input-topic -n kafka
...
### Connectivity issues
```bash
# Verify internal Kafka connectivity
kubectl exec -it kafka-cluster-dual-role-0 -n kafka -- /opt/kafka/bin/kafka-topics.sh --list --bootstrap-server localhost:9092
```
...
Adjust Kafka/Spark memory in YAMLs for smaller machines.
